/**
 * Nombre: delta_046_DA.sql
 * Autor: Ignacio Chelasco
 * User Story 16413: Agregar clausula de riesgo de terceros (Filipinas)
 * Fecha Creacion: 06/12/2021
 */
...
SELECT @IdClausula = IdClausula, @OrdenPredefinido = OrdenPredefinido FROM Clausula WHERE Codigo = 'RISK3'
...
WHERE  IdLocacion IN(SELECT IdLocacion 
                     ...
                     WHERE  Codigo IN(630)) 
       AND IdEstado <> 25002 
       AND P.CodigoPais IN (630) 
       AND ((P.Codigo = '100' AND P.IdTipoGrupoClausula = 3) or
			(P.IdTipoGrupoClausula = 1)) AND OAC.IdEstado <> 25002
...
INNER JOIN Condiciones.dbo.Producto P ON T.IdProducto = P.IdProducto 
	...
	and IdLocacion in (select IdLocacion from Portal.Locacion.Pais where Codigo in(630) and IdEstado <> 25002)
	...
	and CC.IdEstado <> 25002 AND T.Activa = 1)
...
INSERT INTO Leyenda (IdContenidoClausulaRango,IdIdioma,Texto,IdUsuario,FechaCreacion,FechaModificacion,IdEstado)
	...
	AND CCR.Contenido = @Contenido AND T.Activa = 1 GROUP BY CCR.IdContenidoClausulaRango;
